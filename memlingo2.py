...
@app.route('/')
def serve_root():
    return redirect('/pages2/flash.html')
...
# PAGES 파일 서비스
@app.route('/pages2/<path:path>')
def serve_pages(path):
    return send_from_directory('pages2', path)
...
def card_next():
    ...
    if next_row[6] == '0000-00-00 00:00:00':
        ...
        quizlist = ['quizB','quizC','quizD','quizG','quizH','quizI','quizJ','quizK']
        quiz_card = random.choice(quizlist)
        quiz_card_url = './'+quiz_card+'.html'
