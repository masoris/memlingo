...
function click_option(item) { //제시된 여러개의 단어를 클릭한 경우 
    ...
    for (i = 0; i < 100; i++) {
        input = $(i);
        if (input != null && input.style.color == 'white') {
            first_word = input.value;
            first_input = input;
            break;
        }
    }
    if (item.value == first_word) {
...
function get_similar_words_jk(j_word, k_word, esp_txt_words) {
    ...
    postAjaxRequest('/api2/similar-words-jk.api', jsonStr, function (responseJSONStr) {
        ...
        rand_words.push(...esp_txt_words);
        rand_words = [...new Set(rand_words)];
...
window.onload = function () {
    ...
    esp_txt2 = carditem.esp_txt.replace(/,/g, " , ").replace(/\?/g, " ? ").replace(/!/g, " ! ").replace(/~/g, " ~ ").replace(/\./g, " . ").replace(/\(/g, " ( ").replace(/\)/g, " ) ").replace(/"/g, ' " ').replace(/”/g, ' ” ').replace(/=/g, ' = ').replace(/「/g, ' 「 ').replace(/」/g, ' 」 ');
    ...
    for (i = 0; i < esp_txt_words.length; i++) {
        if (arr.includes(esp_txt_words[i])) {
            $('answers').innerHTML = $('answers').innerHTML + "<input id='" + i + "' style='color:black;' type='button' value='" + esp_txt_words[i] + "'>";
        } else {
            $('answers').innerHTML = $('answers').innerHTML + "<input id='" + i + "' style='color:white;' type='button' value='" + esp_txt_words[i] + "'>";
        }
    }
    ...
    get_similar_words_jk(j_word, k_word, esp_txt_words);
