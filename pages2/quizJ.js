...
function get_similar_words_jk(j_word, k_word) {
    ...
    postAjaxRequest('/api/similar-words-jk.api', jsonStr, function (responseJSONStr) {
        ...
        for (k = 0; k < rand_words.length; k++) {
            found = false;
            for (i = 0; i < esp_txt_words.length; i++) {
                if (rand_words[k] == esp_txt_words[i]) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                $('wrong_word').value = rand_words[k];
                break;
            }
        }
...
window.onload = function () {
    ...
    r = Math.floor(Math.random() * (esp_txt_words.length));
    for (i = 0; i < esp_txt_words.length; i++) {
        $('answers').innerHTML = $('answers').innerHTML + "<input id='word_" + i + "' type='button' value='" + esp_txt_words[i] + "'>";
        if (i == r) {
            $('answers').innerHTML = $('answers').innerHTML + "<input id='wrong_word' type='button' value=''>";
        }
    }
    ...
    for (i = 0; i < esp_txt_words.length; i++) {
        $('word_' + i).onclick = function () {
            $(this.id).style.color = 'red';
            var this_id = this.id;
            setTimeout(function () { $(this_id).style.color = 'black'; }, 1000);
        }
    }
    $('wrong_word').onclick = function () {
        $('wrong_word').style.color = 'white';
        $('wrong_word').style.borderColor = 'white';
        $('wrong_word').style.borderStyle = 'hidden';
        $('wrong_word').onclick = function () { };
